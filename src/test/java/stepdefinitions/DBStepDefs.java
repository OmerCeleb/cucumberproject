...
//       DBUtils.executeQuery("SELECT * FROM jhi_user");  // HARD CODED
        DBUtils.executeQuery("SELECT "+columnName+" FROM "+ tableName);  // HARD CODED
...
//        Using while loop
        ...
        System.out.println("There are "+ rowNum + " data in the "+columnName+ " colum");

    }

    @Then("verify table {string} and column {string} contains data {string}")
    public void verify_table_and_column_contains_data(String tableName, String columnName, String data) {
